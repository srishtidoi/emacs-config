...
;; [[file:~/.config/doom/config.org::*Fun][Fun:8]]
(use-package! keycast
...
;; [[file:~/.config/doom/config.org::*Music][Music:1]]
(after! org
...
This action is reversed by `org-music-time-to-seconds'."
  ...
  (defun org-music-mpris-play (file &optional start-time end-time)
    (let ((uri (replace-regexp-in-string " " "%20" (rng-file-name-uri file))))
      (pp uri)
  ...
  (defun org-music-find-file (artist title)
    ...
    (when-let* ((music-folder (expand-file-name org-music-folder))
              (search-folders (or
                               (mapcar (lambda (f) (f-join music-folder f))
                                       (-filter ; look for folders which contain ARTIST
                                        (lambda (content-name)
                                          (and
                                           (s-contains-p artist content-name t)
                                           (eq (file-name-extension content-name) nil)))
                                        (directory-files music-folder)))
                               (list music-folder)))
...
;; [[file:~/.config/doom/config.org::*Music][Music:2]]
(after! org
  ...
  (defun org-music-export (path desc backend _com)
    ...
    (let* ((track-info (org-music-parse-link path))
           ...
           (end-time (nth 2 track-info))
           (emphase (cond ((org-export-derived-backend-p backend 'html)
      ...
      (concat
       (cond ((and start-time end-time)
              (format "%s to %s seconds of " start-time end-time)
              (start-time
               (format "%s seconds into " start-time))))
       (emphase track)
       " by "
       artist))))
;; Music:2 ends here
